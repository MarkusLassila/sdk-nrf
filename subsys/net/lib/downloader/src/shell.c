...
static struct downloader_cfg config = {
...
static char uri[CONFIG_DOWNLOADER_MAX_HOSTNAME_SIZE + CONFIG_DOWNLOADER_MAX_FILENAME_SIZE + 1];
...
static int cmd_dc_init(const struct shell *shell, size_t argc, char **argv)
{
	...
	err = downloader_init(&downloader, &config);
	if (err) {
...
static int cmd_download_get(const struct shell *shell, size_t argc, char **argv)
{
	...
	err = downloader_get(&downloader, &host_config, uri, from);
