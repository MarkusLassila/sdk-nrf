					...
					struct downloader_host_cfg *host_cfg)
{
	...
	*fd = socket(remote_addr->sa_family, type, proto);
	...
	if ((proto == IPPROTO_TLS_1_2 || proto == IPPROTO_DTLS_1_2) &&
		...
		err = socket_sectag_set(*fd, host_cfg->sec_tag_list, host_cfg->sec_tag_count);
		if (err) {
			goto cleanup;
		}
		...
		if (proto == IPPROTO_DTLS_1_2 && host_cfg->cid) {
			printk("enabling CID\n");
				    ...
				    struct downloader_host_cfg *host_cfg)
{
	...
	fam = host_cfg->family ? host_cfg->family : AF_INET6;

	err = dl_socket_host_lookup(hostname, host_cfg->pdn_id, remote_addr, fam);
	if (!err) {
		goto connect;
	} else if (host_cfg->family) {
		LOG_ERR("Host lookup failed for hostname %s, err %d", hostname, err);
		return err;
	}

fallback_ipv4:
	err = dl_socket_host_lookup(hostname, host_cfg->pdn_id, remote_addr, AF_INET);
	if (err) {
		LOG_ERR("Host lookup failed for hostname %s, err %d", hostname, err);
		return err;
	}
...
connect:
	err = dl_socket_create_and_connect(fd, proto, type, port, remote_addr, hostname, host_cfg);
	if (err) {
		if (remote_addr->sa_family == AF_INET6) {
