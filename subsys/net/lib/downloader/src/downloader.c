...
static int transport_init(struct downloader *dl, struct downloader_host_cfg *host_config,
			  const char *uri)
{
	__ASSERT_NO_MSG(dl && dl->transport);

	return dl->transport->init(dl, host_config, uri);
}
...
static void restart_and_suspend(struct downloader *dl)
{
	...
	if (!dl->host_config.keep_connection) {
		transport_close(dl);
		if (!dl->complete) {
			stopped_evt_send(dl);
		}
...
{
	if (dl == NULL || config == NULL || config->callback == NULL || config->buf == NULL ||
	    config->buf_size == 0) {
		return -EINVAL;
	}
...
static int downloader_start(struct downloader *dl, const struct downloader_host_cfg *host_config,
			    const char *uri, size_t from)
{
	...
	if (dl == NULL || host_config == NULL || uri == NULL) {
		return -EINVAL;
	}
	...
	if (is_state(dl, DOWNLOADER_CONNECTED)) {
		...
		err = dl_parse_url_host(uri, hostname, sizeof(hostname));
		if (err) {
	...
	err = dl_parse_url_host(uri, dl->hostname, sizeof(dl->hostname));
	if (err) {
	...
	err = dl_parse_url_file(uri, dl->file, sizeof(dl->file));
	if (err) {
		LOG_ERR("Failed to parse filename, err %d, uri %s", err, uri);
		k_mutex_unlock(&dl->mutex);
		return err;
	}

	dl->host_config = *host_config;
	dl->file_size = 0;
	dl->progress = from;
	dl->buf_offset = 0;
	dl->complete = false;
	...
	{
		if (entry->transport->proto_supported(dl, uri)) {
	...
	if (!dl->transport) {
		if (strstr(uri, "://") == NULL) {
			...
			if (host_config->sec_tag_list && host_config->sec_tag_count) {
				fallback = FALLBACK_HTTPS;
			}

			LOG_WRN("Protocol not specified for %s, attempting %s", uri, fallback);
			STRUCT_SECTION_FOREACH(dl_transport_entry, entry)
			{
		...
		if (!dl->transport) {
			LOG_ERR("Protocol not found for %s", uri);
			k_mutex_unlock(&dl->mutex);
	...
	err = transport_init(dl, &dl->host_config, uri);
	if (err) {
		state_set(dl, DOWNLOADER_CONNECTING, DOWNLOADER_IDLE);
		k_mutex_unlock(&dl->mutex);
...
int downloader_get(struct downloader *dl, const struct downloader_host_cfg *host_config,
		   const char *uri, size_t from)
{
	int rc;
	...
	rc = downloader_start(dl, host_config, uri, from);

	return rc;
}

int downloader_get_with_host_and_file(struct downloader *dl,
				      const struct downloader_host_cfg *host_config,
				      const char *host, const char *file, size_t from)
{
	int rc;

	if (dl == NULL || host_config == NULL || host == NULL || file == NULL) {
		return -EINVAL;
	}
	...
	/* We use the download client buffer to parse the uri */
	if (strlen(host) + strlen(file) + 2 > dl->config.buf_size) {
		LOG_ERR("Download client buffer is not large enough to parse uri");
		k_mutex_unlock(&dl->mutex);
		return -EINVAL;
	}

	snprintf(dl->config.buf, dl->config.buf_size, "%s/%s", host, file);

	rc = downloader_start(dl, host_config, dl->config.buf, from);
