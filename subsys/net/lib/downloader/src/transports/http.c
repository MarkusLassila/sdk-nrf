...
struct transport_params_http {
	...
	struct downloader_transport_http_conf config;
...
static int http_get_request_send(struct downloader *dl)
{
	...
	tls_force_range = (http->sock.proto == IPPROTO_TLS_1_2 && !dl->host_config.set_native_tls &&
			   IS_ENABLED(CONFIG_SOC_SERIES_NRF91X));

	if (dl->host_config.range_override) {
		if (tls_force_range && dl->host_config.range_override > (TLS_RANGE_MAX - 1)) {
			LOG_WRN("Range override > TLS max range, setting to TLS max range");
			dl->host_config.range_override = (TLS_RANGE_MAX - 1);
		}
	...
	if (dl->host_config.range_override) {
		off = dl->progress + dl->host_config.range_override;
		...
		len = snprintf(dl->config.buf, dl->config.buf_size, HTTP_GET_RANGE, dl->file,
			       dl->hostname, dl->progress, off);
		http->ranged = true;
		http->ranged_progress = 0;
		LOG_DBG("Range request up to %d bytes", dl->host_config.range_override);
		goto send;
	} else if (dl->progress) {
		len = snprintf(dl->config.buf, dl->config.buf_size, HTTP_GET_OFFSET, dl->file,
			       dl->hostname, dl->progress);
		http->ranged = false;
	} else {
		len = snprintf(dl->config.buf, dl->config.buf_size, HTTP_GET, dl->file,
			       dl->hostname);
		http->ranged = false;
	}

send:
	if (len < 0 || len > dl->config.buf_size) {
		LOG_ERR("Cannot create GET request, buffer too small");
		return -ENOMEM;
	}

	if (IS_ENABLED(CONFIG_DOWNLOADER_LOG_HEADERS)) {
		LOG_HEXDUMP_DBG(dl->config.buf, len, "HTTP request");
	}

	LOG_DBG("http request:\n%s\n", dl->config.buf);

	err = dl_socket_send(http->sock.fd, dl->config.buf, len);
	if (err) {
...
static int http_header_parse(struct downloader *dl, size_t buf_len)
{
	...
	for (size_t i = 0; i < parse_len; i++) {
		dl->config.buf[i] = tolower(dl->config.buf[i]);
	}

	/* Look for the status code just after "http/1.1 " */
	p = strnstr(dl->config.buf, "http/1.1 ", parse_len);
	if (p) {
		q = strnstr(p, "\r\n", parse_len - (p - dl->config.buf));
		if (q) {
	...
	do {
		if (dl->file_size == 0) {
			if (http->ranged) {
				...
				p = strnstr(p, "/", parse_len - (p - dl->config.buf));
				if (!p) {
					break;
				}
				q = strnstr(p, "\r\n", parse_len - (p - dl->config.buf));
...
static int http_parse(struct downloader *dl, size_t len)
{
	...
	if (!http->header.has_end) {
		...
		if (parsed_len == len) {
			...
			len = len - parsed_len;
			memmove(dl->config.buf, dl->config.buf + parsed_len, len);
			dl->buf_offset = len;
		}

		if (!http->header.has_end) {
			if (dl->config.buf_size == dl->buf_offset) {
	...
	if (dl->progress + len != dl->file_size) {
		if (http->ranged) {
			...
			if (http->ranged_progress < (dl->host_config.range_override
							     ? dl->host_config.range_override
							     : TLS_RANGE_MAX - 1)) {
...
static int dl_http_init(struct downloader *dl, struct downloader_host_cfg *host_cfg,
			const char *uri)
{
	int err;
	struct transport_params_http *http;
	...
	struct downloader_transport_http_conf tmp_config = http->config;

	memset(http, 0, sizeof(struct transport_params_http));
	http->config = tmp_config;

	http->sock.proto = IPPROTO_TCP;
	http->sock.type = SOCK_STREAM;

	if (strncmp(uri, HTTPS, strlen(HTTPS)) == 0 ||
	    (strncmp(uri, HTTP, strlen(HTTP)) != 0 &&
	...
	err = dl_parse_url_port(uri, &http->sock.port);
	if (err) {
...
static int dl_http_connect(struct downloader *dl)
{
	...
	err = dl_socket_configure_and_connect(&http->sock.fd, http->sock.proto, http->sock.type,
					      ...
					      dl->hostname, &dl->host_config);
	if (err) {
		return err;
	}

	err = dl_socket_recv_timeout_set(http->sock.fd, http->config.sock_recv_timeo);
	if (err) {
...
static int dl_http_download(struct downloader *dl)
{
	...
	__ASSERT(dl->buf_offset < dl->config.buf_size, "Buffer overflow");

	LOG_DBG("Receiving up to %d bytes at %p...", (dl->config.buf_size - dl->buf_offset),
		(void *)(dl->config.buf + dl->buf_offset));

	len = dl_socket_recv(http->sock.fd, dl->config.buf + dl->buf_offset,
			     dl->config.buf_size - dl->buf_offset);
	...
	if (http->header.has_end) {
		/* Accumulate progress */
		dl->progress += ret;
		dl_transport_evt_data(dl, dl->config.buf, ret);
...
DLC_TRANSPORT(http, &dl_transport_http);
