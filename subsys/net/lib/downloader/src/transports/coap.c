...
struct transport_params_coap {
	...
	struct downloader_transport_coap_conf config;
...
int coap_block_init(struct downloader *dl, size_t from)
{
	...
	coap_block_transfer_init(&coap->block_ctx, coap->config.block_size, 0);
...
static int coap_parse(struct downloader *dl, size_t len)
{
	...
	err = coap_packet_parse(&response, dl->config.buf, len, NULL, 0);
	if (err) {
...
static int coap_request_send(struct downloader *dl)
{
	...
	err = coap_packet_init(&request, dl->config.buf, dl->config.buf_size, COAP_VER,
	...
	if (!has_pending(dl)) {
		...
		params.max_retransmission = coap->config.max_retransmission;
	...
	err = dl_socket_send(coap->sock.fd, dl->config.buf, request.offset);
	if (err) {
...
static bool dl_coap_proto_supported(struct downloader *dl, const char *uri)
{
...
static int dl_coap_init(struct downloader *dl, struct downloader_host_cfg *host_cfg,
			const char *uri)
{
	int err;
	struct transport_params_coap *coap;
	...
	struct downloader_transport_coap_conf tmp_config = coap->config;
	bool config_set = coap->config_set;

	memset(coap, 0, sizeof(struct transport_params_coap));

	if (config_set) {
		coap->config = tmp_config;
		coap->config_set = config_set;
	} else {
		coap->config.block_size = 5;
		coap->config.max_retransmission = 4;
	}
	...
	err = dl_parse_url_port(uri, &coap->sock.port);
	if (err) {
...
static int dl_coap_download(struct downloader *dl)
{
	...
	len = dl_socket_recv(coap->sock.fd, dl->config.buf + dl->buf_offset,
...
DLC_TRANSPORT(coap, &dl_transport_coap);

int downloader_transport_coap_set_config(struct downloader *dl,
					 struct downloader_transport_coap_conf *conf)
{
	struct transport_params_coap *coap;
