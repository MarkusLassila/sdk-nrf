...
static struct downloader_cfg dlc_config = {
...
static size_t file_size_get(size_t *size)
{
	...
	*size = ext_file_sz;
	return 0;
#endif
	return downloader_file_size_get(&dlc, size);
}
...
static size_t downloaded_size_get(size_t *size)
{
	...
	*size = ext_rcvd_sz;
	return 0;
#endif
	return downloader_downloaded_size_get(&dlc, size);
}
...
static int get_from_offset(const size_t offset)
{
	...
	int err = downloader_get_with_host_and_file(&dlc, &host_config, dl_host, dl_file, offset);
	...
	const enum dfu_target_image_type expected_type)
{
	...
	host_config.pdn_id = pdn_id;
	host_config.range_override = fragment_size;
	...
	err = downloader_get_with_host_and_file(&dlc, &host_config, dl_host, dl_file, 0);
	if (err != 0) {
		atomic_clear_bit(&flags, FLAG_DOWNLOADING);
		(void)dl_cancel();
		return err;
...
static int fota_download_object_init(void)
{
	 ...
	 * if configured.
	 */
	host_config.native_tls = CONFIG_FOTA_DOWNLOAD_NATIVE_TLS;
#endif
