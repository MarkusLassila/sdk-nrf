...
static char dlc_buf[CONFIG_LWM2M_CARRIER_FIRMWARE_DOWNLOAD_BUF_SIZE];
...
{
	struct downloader_host_cfg config = {
	...
	return downloader_get(&http_downloader, &config, uri, from);
}
...
int lwm2m_os_download_init(lwm2m_os_download_callback_t lib_callback)
{
	...
	return downloader_init(&http_downloader, &dlc_config);
}
