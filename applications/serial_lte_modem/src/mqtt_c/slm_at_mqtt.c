				 ...
				 uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		err = at_parser_string_get(parser, 1, mqtt_clientid, &clientid_sz);
		if (err) {
			return err;
		}
		if (param_count > 2) {
				  ...
				  uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		if (op == MQTTC_CONNECT || op == MQTTC_CONNECT6)  {
			...
			err = at_parser_string_get(parser, 2, mqtt_username, &username_sz);
			if (err) {
				return err;
			} else {
			...
			err = at_parser_string_get(parser, 3, mqtt_password, &password_sz);
			if (err) {
				return err;
			} else {
			...
			err = at_parser_string_get(parser, 4, mqtt_broker_url, &url_sz);
			if (err) {
				return err;
			}
				  ...
				  uint32_t param_count)
{
	...
	switch (cmd_type) {
	case AT_PARSER_CMD_TYPE_SET:
		err = at_parser_string_get(parser, 1, pub_topic, &topic_sz);
		if (err) {
			return err;
		}
		pub_msg[0] = '\0';
		if (param_count > 2) {
			err = at_parser_string_get(parser, 2, pub_msg, &msg_sz);
			if (err) {
				return err;
			}
		}
				    ...
				    struct at_parser *parser, uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		if (param_count == 3) {
			err = at_parser_string_get(parser, 1, topic, &topic_sz);
			if (err < 0) {
				return err;
			}
