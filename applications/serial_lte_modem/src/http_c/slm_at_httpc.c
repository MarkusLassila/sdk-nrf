				   ...
				   struct at_parser *parser, uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		if (op == HTTPC_CONNECT || op == HTTPC_CONNECT6) {
			err = at_parser_string_get(parser, 2, httpc.host, &host_sz);
			if (err) {
				return err;
			}
				   ...
				   struct at_parser *parser, uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		err = at_parser_string_get(parser, 1, slm_data_buf, &size);
		if (err < 0) {
		...
		size = HTTPC_RES_LEN;
		err = at_parser_string_get(parser, 2, slm_data_buf + offset, &size);
		if (err < 0) {
		...
		if (param_count >= 4) {
			...
			err = at_parser_string_get(parser, 3, slm_data_buf + offset, &size);
			if (err == 0 && size > 0) {
		...
		if (param_count >= 5) {
			...
			err = at_parser_string_get(parser, 4, slm_data_buf + offset, &size);
			if (err == 0 && size > 0) {
