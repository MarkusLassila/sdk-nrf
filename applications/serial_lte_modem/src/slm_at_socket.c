				      ...
				      struct at_parser *parser, uint32_t)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		if (op == AT_SOCKETOPT_SET) {
			...
			if (err == -EOPNOTSUPP) {
				err = at_parser_string_get(parser, 3, value_str, &size);
				if (err) {
					return err;
				}
			     ...
			     uint32_t)
{
	...
	switch (cmd_type) {
	case AT_PARSER_CMD_TYPE_SET:
		err = at_parser_string_get(parser, 1, url, &size);
		if (err) {
			return err;
		}
			  ...
			  uint32_t param_count)
{
	...
	if (at_parser_string_get(parser, 0, at_cmd, &at_cmd_size)){
		return -EINVAL;
	}
	...
	case AT_PARSER_CMD_TYPE_SET:
		if (param_count > 1) {
			size = sizeof(data);
			err = at_parser_string_get(parser, 1, data, &size);
			if (err) {
				return err;
			}
			    ...
			    uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		size = sizeof(udp_url);
		err = at_parser_string_get(parser, 1, udp_url, &size);
		if (err) {
			return err;
		}
				 ...
				 uint32_t)
{
	...
	switch (cmd_type) {
	case AT_PARSER_CMD_TYPE_SET:
		err = at_parser_string_get(parser, 1, host, &size);
		if (err) {
			return err;
		}
