...
static void ncell_meas_mon(const char *notify)
{
	...
	/* parse Cell ID */
	size = sizeof(cid);
	err = at_parser_string_get(&parser, 2, cid, &size);
	if (err) {
		goto exit;
	}
	...
	/* parse PLMN */
	size = sizeof(plmn);
	err = at_parser_string_get(&parser, 3, plmn, &size);
	if (err) {
		goto exit;
	}
	...
	/* parse TAC */
	size = sizeof(tac);
	err = at_parser_string_get(&parser, 4, tac, &size);
	if (err) {
		goto exit;
	}
				   ...
				   struct at_parser *parser, uint32_t param_count)
{
	...
	if (wifi_pos) {
		...
		++param_idx) {
			...
			err = at_parser_string_get(parser, param_idx, mac_addr_str, &len);
