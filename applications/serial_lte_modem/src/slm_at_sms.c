...
static int handle_at_sms(enum at_parser_cmd_type cmd_type, struct at_parser *parser,
			 uint32_t)
{
	int err = -EINVAL;
	uint16_t op;
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		if (op == AT_SMS_STOP) {
			...
			err = at_parser_string_get(parser, 2, number, &size);
			if (err) {
				return err;
			}
			size = MAX_CONCATENATED_MESSAGE * SMS_MAX_PAYLOAD_LEN_CHARS;
			err = at_parser_string_get(parser, 3, message, &size);
			if (err) {
				return err;
			}
