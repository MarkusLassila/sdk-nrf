		      ...
		      uint32_t param_count)
{
	...
	ret = at_parser_string_get(parser, 2, apn, &size);
	if (ret) {
		return ret;
	}
			   ...
			   uint32_t param_count)
{
	...
	ret = at_parser_string_get(parser, 2, size_buf, &size);
			      ...
			      uint32_t param_count)
{
	...
	ret = at_parser_string_get(parser, 2, device_type, &size);
	if (ret) {
		return ret;
	}
				   ...
				   uint32_t param_count)
{
	...
	ret = at_parser_string_get(parser, 2, hardware_version, &size);
	if (ret) {
		return ret;
	}
			       ...
			       uint32_t param_count)
{
	...
	ret = at_parser_string_get(parser, 2, service_code, &size);
	if (ret) {
		return ret;
	}
			  ...
			  uint32_t param_count)
{
	...
	ret = at_parser_string_get(parser, 2, binding, &size);
	if (ret) {
		return ret;
	}
...
static int do_cfg_server_enable(enum at_parser_cmd_type, struct at_parser *parser,
				uint32_t param_count)
{
	if (param_count == 2) {
		rsp_send("\r\n#XCARRIERCFG: %u\r\n",
...
static int do_cfg_uri(enum at_parser_cmd_type, struct at_parser *parser,
		      uint32_t param_count)
{
	if (param_count == 2) {
	...
	ret = at_parser_string_get(parser, 2, server_uri, &size);
	if (ret) {
		return ret;
	}
