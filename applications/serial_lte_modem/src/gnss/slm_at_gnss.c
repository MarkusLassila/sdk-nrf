...
AT_CMD_CUSTOM(xgps_set, "AT#XGPS=", handle_at_gps);
AT_CMD_CUSTOM(xgps_read, "AT#XGPS?", handle_at_gps);
static int handle_at_gps(char *buf, size_t len, char *at_cmd)
{
	int err = -EINVAL;
	uint16_t op;
	uint16_t interval;
	...
	case AT_CMD_TYPE_SET_COMMAND:
		...
		err = at_params_unsigned_short_get(list, OP_IDX, &op);
		if (err) {
			return err;
		}
		if (op == GPS_START) {
			...
			err = at_params_unsigned_short_get(
				list, CLOUD_ASSISTANCE_IDX, &gnss_cloud_assistance);
			...
			err = at_params_unsigned_short_get(list, INTERVAL_IDX, &interval);
...
AT_CMD_CUSTOM(xgpsdel, "AT#XGPSDEL", handle_at_gps_delete);
static int handle_at_gps_delete(char *buf, size_t len, char *at_cmd)
{
	int err = -EINVAL;
	uint32_t mask;
	const struct at_param_list *list = slm_get_at_param_list(at_cmd);
	...
	switch (cmd_type) {
	case AT_CMD_TYPE_SET_COMMAND:
		err = at_params_unsigned_int_get(list, 1, &mask);
