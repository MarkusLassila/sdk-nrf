		       ...
		       uint32_t param_count)
{
	...
	/* Parse AT command */
	ret = at_parser_string_get(parser, 2, username, &sz_username);
	if (ret || strlen(username) == 0) {
		strcpy(username, FTP_USER_ANONYMOUS);
		strcpy(password, FTP_PASSWORD_ANONYMOUS);
	} else {
		ret = at_parser_string_get(parser, 3, password, &sz_password);
		if (ret) {
			return -EINVAL;
		}
	}
	ret = at_parser_string_get(parser, 4, hostname, &sz_hostname);
	if (ret) {
		return ret;
	}
	if (param_count > 5) {
...
static int do_ftp_verbose(enum at_parser_cmd_type, struct at_parser *parser, uint32_t)
{
	...
	ret = at_parser_string_get(parser, 2, vb_mode, &sz_mode);
	if (ret) {
		return ret;
	}
...
static int do_ftp_ls(enum at_parser_cmd_type, struct at_parser *parser, uint32_t param_count)
{
	...
	if (param_count > 2) {
		ret = at_parser_string_get(parser, 2, options, &sz_options);
		if (ret) {
			return ret;
		}
	}
	...
	if (param_count > 3) {
		ret = at_parser_string_get(parser, 3, filepath, &sz_filepath);
		if (ret) {
			return ret;
		}
	}
...
static int do_ftp_cd(enum at_parser_cmd_type, struct at_parser *parser, uint32_t)
{
	...
	sz_filepath = SLM_MAX_FILEPATH;
	ret = at_parser_string_get(parser, 2, filepath, &sz_filepath);
	if (ret) {
		return ret;
	}
...
static int do_ftp_rename(enum at_parser_cmd_type, struct at_parser *parser, uint32_t)
{
	...
	ret = at_parser_string_get(parser, 3, file_new, &sz_file_new);
	if (ret) {
		return ret;
	}
		      ...
		      uint32_t param_count)
{
	...
	if (param_count == 3) {
		...
		err = at_parser_string_get(parser, 3, data, &size);
		if (err) {
			return err;
		}
		       ...
		       uint32_t param_count)
{
	...
	if (param_count == 2) {
		...
		err = at_parser_string_get(parser, 2, data, &size);
		if (err) {
			return err;
		}
