			  ...
			  uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		size = sizeof(url);
		err = at_parser_string_get(parser, 2, url, &size);
		if (err) {
			return err;
		}
		...
		err = at_parser_string_get(parser, 4, filepath, &size);
		if (err) {
			return err;
		}
		if (param_count > 5) {
			size = sizeof(mode);
			err = at_parser_string_get(parser, 5, mode, &size);
			if (err) {
				return err;
			}
		...
		if (op == TFTP_GET) {
			...
			size = sizeof(data);
			err = at_parser_string_get(parser, 6, data, &size);
			if (err) {
				return err;
			}
			if (op == TFTP_PUT) {
