 ...
 */
int load_credential_cb(const char *key, size_t len, settings_read_cb read_cb, void *cb_arg,
				void *param)
{
	int err;
	...
	*lcp->credential_flags |= 1 << type;
	...
	/* Update buf to accommodiate other credentials under the sec_tag. */
 ...
 */
int list_credentials_cb(const char *key, size_t len, settings_read_cb read_cb, void *cb_arg,
				void *param)
{
	ARG_UNUSED(len);
	ARG_UNUSED(read_cb);
...
/* Called for every (slm-keys/<sec_tag>/)<type>. */
int get_setting_len_cb(const char *key, size_t len, settings_read_cb read_cb, void *cb_arg,
				void *param)
{
	ARG_UNUSED(key);
	ARG_UNUSED(read_cb);
...
int get_credentials_len(sec_tag_t sec_tag, size_t *len)
{
...
struct tls_cred_buf *get_tls_cred_buf(sec_tag_t sec_tag)
{
...
int unload_tls_cred_buf(sec_tag_t sec_tag)
{
