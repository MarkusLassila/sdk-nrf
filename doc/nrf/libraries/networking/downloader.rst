...
.. code-block:: c
   ...
   struct downloader_cfg dlc_conf = {
         .callback = dlc_callback,
         .buf = dlc_buf,
         .buf_size,
   };

   err = downloader_init(&dlc, &dls_config);
   if (err) {
...
.. code-block:: c
   ...
   struct downloader_transport_http_conf dl_transport_http_conf = {
         .sock_recv_timeo = 1000,
   };
   ...
   err = downloader_transport_http_set_config(&dlc, &dl_transport_http_conf);
   if (err) {
         printk("failed to set http transport params failed, err %d\n", err);
   }

   err = downloader_get(&dlc, &dlc_host_cgf, "https://myserver.com/path/to/file.txt");
   if (err) {
         printk("downloader start failed, err %d\n", err);
   }

   /* Wait for download to complete */
   k_sem_take(&dlc_sem, K_FOREVER);
...
The library requires the host server to provide a Content-Range field in the HTTP GET header when using HTTPS with the nRF91 Series devices.
