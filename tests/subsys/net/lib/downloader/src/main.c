...
#define NO_PROTO_URI "server.com/path/to/file.end"
#define HTTP_URI "http://server.com:80/path/to/file.end"
#define HTTP_URI_FILE2 "http://server.com/path/to/file2.end"
#define HTTP_URI_HOST2 "http://server2.com/path/to/file.end"
#define HTTPS_URI "https://server.com/path/to/file.end"
#define COAP_URI "coap://server.com/path/to/file.end"
#define COAPS_URI "coaps://server.com/path/to/file.end"
#define BAD_URI "bad://server.com/path/to/file.end"
#define BAD_URI2 "bad:/server.com:80/path/to/file.end"
#define BAD_URI_NO_FILE "http://server.com"
...
static int dlc_callback(const struct downloader_evt *event);
static int dlc_callback_abort(const struct downloader_evt *event);

static struct downloader dlc;

char dlc_buf[2048];
struct downloader_cfg dlc_config = {
	.callback = dlc_callback,
	.buf = dlc_buf,
	.buf_size = sizeof(dlc_buf),
};

struct downloader_cfg dlc_config_small_buffer = {
	.callback = dlc_callback,
	.buf = dlc_buf,
	.buf_size = 32,
};

struct downloader_cfg dlc_config_cb_abort = {
	.callback = dlc_callback_abort,
	.buf = dlc_buf,
	.buf_size = sizeof(dlc_buf),
};

static struct downloader_host_cfg dlc_host_cfg = {
	.pdn_id = 1,
	.keep_connection = true,
};

int sec_tags[] = {1, 2, 3};
static struct downloader_host_cfg dlc_host_cfg_w_sec_tags = {
	.pdn_id = 1,
	.sec_tag_list = sec_tags,
	.sec_tag_count = ARRAY_SIZE(sec_tags),
};

static struct downloader_host_cfg dlc_host_cfg_w_sec_tags_and_cid = {
	.pdn_id = 1,
	.sec_tag_list = sec_tags,
	.sec_tag_count = ARRAY_SIZE(sec_tags),
	.cid = true,
...
{
	TEST_ASSERT_EQUAL(FD, sock);
	TEST_ASSERT(sizeof(dlc_buf) >= max_len);
...
static const char *dlc_event_id_str(int evt_id)
{
	switch (evt_id) {
...
static int dlc_callback(const struct downloader_evt *event)
{
	...
	printk("event: %s ", dlc_event_id_str(event->id));
...
static struct downloader_evt dlc_wait_for_event(enum downloader_evt_id event,
						     k_timeout_t timeout)
{
	size_t read;
...
void test_downloader_init_einval(void)
{
	...
	struct downloader_cfg config = {
		.buf = buf,
		.buf_size = ARRAY_SIZE(buf),
		.callback = dlc_callback,
	};
	struct downloader_cfg config_no_buf = {
		.buf = NULL,
		.buf_size = ARRAY_SIZE(buf),
		.callback = dlc_callback,
	};
	struct downloader_cfg config_no_buf_len = {
		.buf = buf,
		.buf_size = 0,
		.callback = dlc_callback,
	};
	struct downloader_cfg config_no_cb = {
		.buf = buf,
		.buf_size = ARRAY_SIZE(buf),
		.callback = NULL,
	};

	err = downloader_init(NULL, &config);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_init(&dlc, NULL);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_init(&dlc, &config_no_buf);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_init(&dlc, &config_no_buf_len);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_init(&dlc, &config_no_cb);
	TEST_ASSERT_EQUAL(-EINVAL, err);
}
...
void test_downloader_get_eperm(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg, HTTP_URI, 0);
...
void test_downloader_get_http(void)
{
	...
	err = downloader_init(&dlc, &dlc_config);
	TEST_ASSERT_EQUAL(0, err);
	...
	evt = dlc_wait_for_event(DOWNLOADER_EVT_DONE, K_SECONDS(3));

	downloader_deinit(&dlc);
	dlc_wait_for_event(DOWNLOADER_EVT_DEINITIALIZED, K_SECONDS(1));
}
...
void test_downloader_get_https(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg_w_sec_tags, HTTPS_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_http_connect_enetunreach(void)
{
	...
	err = downloader_init(&dlc, &dlc_config_cb_abort);
	TEST_ASSERT_EQUAL(0, err);
	...
	evt = dlc_wait_for_event(DOWNLOADER_EVT_ERROR, K_SECONDS(1));
...
void test_downloader_get_http_getaddr_failed(void)
{
	...
	evt = dlc_wait_for_event(DOWNLOADER_EVT_ERROR, K_SECONDS(3));
...
void test_downloader_get_default_proto_http(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg, NO_PROTO_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_default_proto_https(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg_w_sec_tags, NO_PROTO_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_coap(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg, COAP_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_coaps(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg_w_sec_tags, COAPS_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_coaps_setsockopt_fail_sndtimeo(void)
{
	int err;
	struct downloader_evt evt;
	struct downloader_transport_coap_conf coap_conf = {
	...
	err = downloader_transport_coap_set_config(&dlc, &coap_conf);
	TEST_ASSERT_EQUAL(0, err);
	...
	err = downloader_cancel(&dlc);
	evt = dlc_wait_for_event(DOWNLOADER_EVT_STOPPED, K_SECONDS(3));
...
void test_downloader_get_coaps_setsockopt_fail_cid(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg_w_sec_tags_and_cid, COAPS_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_einval(void)
{
	...
	err = downloader_get(NULL, &dlc_host_cfg, HTTP_URI, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get(&dlc, NULL, HTTP_URI, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get(&dlc, &dlc_host_cfg, NULL, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get(&dlc, &dlc_host_cfg, BAD_URI_NO_FILE, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	/* https uri, no sec tag specified */
	err = downloader_get(&dlc, &dlc_host_cfg, HTTPS_URI, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	/* coaps uri, no sec tag specified */
	err = downloader_get(&dlc, &dlc_host_cfg, COAPS_URI, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);
...
void test_downloader_get_eprotonotsupport(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg, BAD_URI, 0);
...
void test_downloader_get_two_files_same_host(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg, HTTP_URI_FILE2, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_two_files_different_host(void)
{
	...
	err = downloader_get(&dlc, &dlc_host_cfg, HTTP_URI_HOST2, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_hdr_and_payload(void)
{
	...
	evt = dlc_wait_for_event(DOWNLOADER_EVT_FRAGMENT, K_SECONDS(3));
...
void test_downloader_get_ipv4_specific_ok(void)
{
	int err;
	struct downloader_evt evt;
	static struct downloader_host_cfg dlc_host_cfg_ipv4 = {
		.pdn_id = 1,
		.family = AF_INET,
	};
	...
	err = downloader_get(&dlc, &dlc_host_cfg_ipv4, HTTP_URI, 0);
	TEST_ASSERT_EQUAL(0, err);
...
void test_downloader_get_with_host_and_file_einval(void)
{
	...
	err = downloader_init(&dlc, &dlc_config_small_buffer);
	TEST_ASSERT_EQUAL(0, err);

	err = downloader_get_with_host_and_file(NULL, &dlc_host_cfg, HTTP_HOST, FILE_PATH, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get_with_host_and_file(&dlc, NULL, HTTP_HOST, FILE_PATH, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get_with_host_and_file(&dlc, &dlc_host_cfg, NULL, FILE_PATH, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get_with_host_and_file(&dlc, &dlc_host_cfg, HTTP_HOST, NULL, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	err = downloader_get_with_host_and_file(&dlc, &dlc_host_cfg, HTTP_HOST, FILE_PATH_LONG, 0);
	TEST_ASSERT_EQUAL(-EINVAL, err);
...
void test_downloader_get_with_host_and_file_eperm(void)
{
	...
	err = downloader_get_with_host_and_file(&dlc, &dlc_host_cfg, HTTP_HOST, FILE_PATH, 0);
	TEST_ASSERT_EQUAL(-EPERM, err);
}
...
void test_downloader_file_size_get_einval(void)
{
	...
	err = downloader_file_size_get(&dlc, NULL);
	TEST_ASSERT_EQUAL(-EINVAL, err);
...
void test_downloader_file_size_get_eperm(void)
{
	...
	err = downloader_file_size_get(&dlc, &fs);
	TEST_ASSERT_EQUAL(-EPERM, err);
}
...
void test_downloader_downloaded_size_get_einval(void)
{
	...
	err = downloader_downloaded_size_get(&dlc, NULL);
	TEST_ASSERT_EQUAL(-EINVAL, err);
...
void test_downloader_downloaded_size_get_eperm(void)
{
	...
	err = downloader_downloaded_size_get(&dlc, &fs);
	TEST_ASSERT_EQUAL(-EPERM, err);
}
